package sprintbot;

public class MyConstants {
    public static int ROBOT_COUNT_OFFSET = 0; // will occupy channels 0 - 20
    public static int SPAWN_TYPE_OFFSET = 21; // will occupy channels 21 - 42
    public static int ATTACK_LOCATION = 43; //will occupy channels 43 - 44
    public static int POINTS_OF_INTEREST_OFFSET = 45; // will occupy channels 45 - 90; 
    public static int ALONE_TOWER_INDEX = 91; //stores the index of the tower we want to send launcher to protect
}